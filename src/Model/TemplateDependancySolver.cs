...
namespace SideLoader.Model
{
    ...
    //        var graph = new HashSet<T>(allTemplates.Where(it => it.DoesTargetExist));
    ...
    //            foreach (var incoming in dependencies.Where(it => it.TargetID.Equals(outgoing.AppliedID)).ToList())
    //            {
    //                // in this design, each node can have only one edge, so we dont need to check for more dependencies.
    //                dependencies.Remove(incoming);
    //                graph.Add(incoming);
